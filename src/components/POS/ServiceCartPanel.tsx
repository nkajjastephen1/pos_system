...
import { ServiceCartItem as ServiceCartItemComponent } from './ServiceCartItem';
import { Button } from '../ui/Button';
import { useServiceSales } from '../../context/ServiceSalesContext';
import { calculateTotal } from '../../utils/calculations';
import { formatCurrency } from '../../utils/formatters';

interface ServiceCartPanelProps {
  onCheckout: () => void;
}

export function ServiceCartPanel({ onCheckout }: ServiceCartPanelProps) {
  const { cart, updateCartAmount, removeFromCart, clearCart } = useServiceSales();
  ...
  if (cart.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-slate-500 dark:text-slate-400 p-8 text-center border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        ...
        <p className="text-sm">
          Select services to add them to the cart.
        </p>
      </div>
    );
  }
